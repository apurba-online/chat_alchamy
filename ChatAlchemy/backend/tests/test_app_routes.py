...
def test_health_and_data_routes():
    with TestClient(app) as client:
        ...
        assert "claim-to-evidence link validation" in body["capabilities"]
        assert isinstance(body["server_llm_configured"], bool)
        assert "environment" in body
        assert "deployment_commit" in body
        assert "deployment_branch" in body
...
def test_openai_network_failure_is_mapped_to_clean_service_error():
    request = httpx.Request("POST", "https://api.openai.com/v1/responses")
    upstream = httpx.ConnectError("network unavailable", request=request)
