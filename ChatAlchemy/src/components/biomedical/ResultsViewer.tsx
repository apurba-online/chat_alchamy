...
interface Results {
  ...
  enrichmentResults?: Array<{ term: string; genes: string[]; pValue: number; adjustedPValue: number }>;
...
export function ResultsViewer({
  ...
  const transfer = () => {
    ...
    try {
      ...
      const context = `# Document Evidence Context\n\n${results.paperSummary ? `## Literature Summary\n${results.paperSummary}\n\n` : ''}## Live Evidence Analysis\n${results.explanation}\n\n## Genes\n${(results.genes || []).join(', ') || 'None'}\n\n## Enrichment\n${(results.enrichmentResults || []).slice(0, 10).map(item => `- ${item.term}: q=${item.adjustedPValue.toExponential(2)}; genes=${item.genes.join(', ')}`).join('\n') || 'No enrichment terms'}\n\nThis context came from ChatAlchemy Document Lab and its live evidence workflow.`;
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-400">
            ...
            <span>·</span>
            <span>{results.enrichmentResults?.length || 0} enrichment results</span>
      ...
      {!!results.clusters?.length && (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-white"><Network className="h-5 w-5 text-indigo-500" />Gene clusters</h3>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {results.clusters.map(cluster => <div key={cluster.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40"><div className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Cluster {cluster.id}</div><div className="flex flex-wrap gap-1.5">{cluster.genes.map(gene => <span key={gene} className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">{gene}</span>)}</div>{cluster.description && <p className="mt-3 text-xs leading-5 text-slate-500 dark:text-slate-400">{cluster.description}</p>}</div>)}
          </div>
        </section>
      )}
      ...
      {!!results.enrichmentResults?.length && (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-white"><FlaskConical className="h-5 w-5 text-indigo-500" />Disease-set enrichment</h3>
          <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
            {results.enrichmentResults.slice(0, 15).map((result, index) => <div key={result.term} className={`flex flex-col justify-between gap-2 px-4 py-3 text-sm md:flex-row md:items-center ${index ? 'border-t border-slate-100 dark:border-slate-800' : ''}`}><span className="font-medium text-slate-800 dark:text-slate-100">{result.term}</span><span className="text-xs text-slate-500 dark:text-slate-400">q={result.adjustedPValue.toExponential(2)} · {result.genes.join(', ')}</span></div>)}
          </div>
        </section>
      )}
    </div>
