...
export default function App(){
  ...
  const [darkMode,setDarkMode]=useState(()=>localStorage.getItem('theme')==='dark'||(!localStorage.getItem('theme')&&matchMedia('(prefers-color-scheme: dark)').matches));
  ...
  const newChat=()=>{
    const c=createNewChat();c.messages=[{id:uuidv4(),role:'assistant',content:'Welcome to **ChatAlchemy**. Ask about your uploaded data, live pharmaceutical databases, clinical trials, genes, diseases, compounds, or continue an analysis from the Biomedical workspace.',timestamp:new Date()}];saveChat(c);setCurrentChat({...c});setShowBiomedical(false);setShowChat(true);setError(null);
  };
  ...
  const fileUploaded=()=>{setLoadedFiles(getLoadedFiles());setError(null);if(currentChat){const m:Message={id:uuidv4(),role:'assistant',content:`Uploaded data is ready. I can filter, count, tabulate, chart, or combine candidate drug names with live evidence.`,timestamp:new Date()};const c={...currentChat,messages:[...currentChat.messages,m],updatedAt:new Date()};setCurrentChat(c);saveChat(c)}};
  const clearUploaded=()=>{void clearData().then(()=>setLoadedFiles([]));if(currentChat){const m:Message={id:uuidv4(),role:'assistant',content:'Uploaded data has been cleared. Live biomedical sources remain available.',timestamp:new Date()};const c={...currentChat,messages:[...currentChat.messages,m],updatedAt:new Date()};setCurrentChat(c);saveChat(c)}};

  const send=async(content:string)=>{
    if(!currentChat)return;
    const user:Message={id:uuidv4(),role:'user',content,timestamp:new Date()};
    let c:Chat={...currentChat,messages:[...currentChat.messages,user],updatedAt:new Date()};
    if(currentChat.messages.length===1)c={...c,name:await generateChatName(content)};
    setCurrentChat(c);saveChat(c);setLoading(true);setError(null);
    try{
      const uploaded=await searchKnowledgeBase(content);
      const isDataOutput=!!(uploaded.tableData||uploaded.chartData)&&!LIVE_TERMS.test(content);
      let assistant:Message;
      if(isDataOutput){
        assistant={id:uuidv4(),role:'assistant',content:`I found **${uploaded.matchCount} matching record${uploaded.matchCount===1?'':'s'}** in your uploaded data.`,timestamp:new Date(),tableData:uploaded.tableData,chartData:uploaded.chartData};
      }else{
        const conversation=c.messages.slice(-10).map(m=>({role:m.role,content:m.content}));
        let data:QueryResponse;
        if(LIVE_TERMS.test(content)){
          const candidates=await getCandidateDrugEvidence();
          data=await queryLive(content,conversation,candidates);
        }else{
          data=await chatApi(conversation,uploaded.text||undefined);
        }
        assistant=responseMessage(data);
        if(uploaded.matchCount>0&&!assistant.tableData&&!assistant.chartData&&!LIVE_TERMS.test(content)) assistant.content+=`\n\nI also matched **${uploaded.matchCount} record(s)** in your uploaded data for this question.`;
      }
      c={...c,messages:[...c.messages,assistant],updatedAt:new Date()};setCurrentChat(c);saveChat(c);
    }catch(e){const msg=e instanceof Error?e.message:'Unknown request error';setError(msg);const assistant:Message={id:uuidv4(),role:'assistant',content:'I could not complete that request. I did not substitute an unsupported biomedical answer.',timestamp:new Date(),warnings:[msg]};c={...c,messages:[...c.messages,assistant],updatedAt:new Date()};setCurrentChat(c);saveChat(c)}finally{setLoading(false)}
  };
